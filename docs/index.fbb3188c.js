...
async function showMessages() {
    try {
        const { messages  } = await _api.API.getMessages();
        const slicedMessages = messages.slice(0, 2);
        slicedMessages.forEach((item)=>{
            _view.renderPartnersMessages(item);
        });
    } catch (error) {
        alert(error);
    }
...
showMessages();
_view.UI_ELEMENTS.MODALS_OVERLAYS.forEach((item, i)=>{
    item.addEventListener('click', (e)=>{
        const target = e.target;
        const isMatches = target.matches('.modal-overlay') || target.matches('.modal__close');
        if (isMatches) _modal.closeModal(i);
    });
});
_view.UI_ELEMENTS.SEND_MESSAGE_FORM.addEventListener('submit', (e)=>{
    e.preventDefault();
    const inputValue = _view.UI_ELEMENTS.MESSAGE_INPUT.value;
    if (_view.isEmpty(inputValue)) return;
    _view.renderMyMessage(inputValue);
    _view.clearInput(_view.UI_ELEMENTS.MESSAGE_INPUT);
});
_view.UI_ELEMENTS.AUTH_FORM.addEventListener('submit', (e)=>{
    e.preventDefault();
    const inputValue = _view.UI_ELEMENTS.AUTH_EMAIL_INPUT.value;
    if (_view.isEmpty(inputValue)) return;
    _api.API.sendEmail(inputValue);
    _view.clearInput(_view.UI_ELEMENTS.AUTH_EMAIL_INPUT);
    _modal.closeModal();
    _modal.openModal(1);
});
_view.UI_ELEMENTS.CONFIRM_FORM.addEventListener('submit', (e)=>{
    e.preventDefault();
    const inputValue = _view.UI_ELEMENTS.CONFIRM_INPUT.value;
    if (_view.isEmpty(inputValue)) return;
    _jsCookieDefault.default.set('auth-key', inputValue);
    _view.clearInput(_view.UI_ELEMENTS.CONFIRM_INPUT);
    _modal.closeModal(1);
    _modal.openModal(2);
});
_view.UI_ELEMENTS.NAME_FORM.addEventListener('submit', (e)=>{
    e.preventDefault();
    const inputValue = _view.UI_ELEMENTS.NAME_INPUT.value;
    if (_view.isEmpty(inputValue)) return;
    _api.API.sendName(inputValue);
    _view.clearInput(_view.UI_ELEMENTS.NAME_INPUT);
    _modal.closeModal(2);
});
_view.UI_ELEMENTS.SETTINGS_BUTTON.addEventListener('click', ()=>{
    _api.API.me().then((response)=>{
        if (response.ok) alert('Вы уже авторизованы');
        else _modal.openModal();
    });
});

},{"js-cookie":"c8bBu","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3","./api":"6yDOL","./modal":"guy4I","./view":"2GA9o"}],"c8bBu":[function(require,module,exports) {
...
const API = {
    ...
    sendName (name) {
        ...
        return fetch(URL, {
            ...
            headers: {
                'Content-Type': 'application/json;charset=utf-8',
                Authorization: `Bearer ${_jsCookieDefault.default.get('auth-key')}`
            },
            body: JSON.stringify({
                name: name
            })
    ...
    async getMessages () {
        ...
        else throw new Error(`Данные небыли получены, ошибка ${response.status} ${response.statusText}`);
    },
    me () {
...
},{"js-cookie":"c8bBu","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"guy4I":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
const openModal = (i = 0)=>{
    _viewJs.UI_ELEMENTS.MODALS_OVERLAYS[i].classList.add('modal-overlay--open');
};
const closeModal = (i = 0)=>{
    _viewJs.UI_ELEMENTS.MODALS_OVERLAYS[i].classList.remove('modal-overlay--open');
};

},{"./view.js":"2GA9o","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"2GA9o":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "UI_ELEMENTS", ()=>UI_ELEMENTS
);
parcelHelpers.export(exports, "renderMyMessage", ()=>renderMyMessage
);
parcelHelpers.export(exports, "renderPartnersMessages", ()=>renderPartnersMessages
);
...
function renderMyMessage(messageText) {
    ...
    messageTemplate.querySelector('.my-message__time').textContent = _dateFns.format(new Date(), 'HH:mm');
    UI_ELEMENTS.CHAT_BODY.prepend(messageTemplate);
}
function renderPartnersMessages({ text , user , createdAt  }) {
    ...
    messageTemplate.querySelector('.partners-message__time').textContent = _dateFns.format(_dateFns.parseISO(createdAt), 'HH:mm');
    UI_ELEMENTS.CHAT_BODY.prepend(messageTemplate);
}
